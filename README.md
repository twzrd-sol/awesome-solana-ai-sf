...
- [TWZRD Agent Intel](https://intel.twzrd.xyz) - Trust-scoring MCP server for x402 agents on Solana. Free preflight check returns a reputation score for any agent wallet; paid call returns a signed USDC trust receipt settled on-chain in under 1 second. Streamable-HTTP MCP endpoint at `https://intel.twzrd.xyz/mcp`. Enables agents in multi-agent systems to verify counterparty trust before executing transactions.
